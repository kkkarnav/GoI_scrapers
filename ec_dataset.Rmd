...
states <- c("Andhra_Pradesh", "Bihar", "Chhattisgarh", "Gujarat", "Haryana", "Himachal_Pradesh", "Jharkhand", "Karnataka", "Kerala", "Madhya_Pradesh", "Maharashtra", "Orissa", "Punjab", "Rajasthan", "Tamil_Nadu", "Telangana", "Uttar_Pradesh", "Uttarakhand")
csvs <- c("_ec_pdf_links.csv", "_ec_compiled_data.csv", "_ec_timelinedata.csv")

read_csvs <- function(csv_type) {
  file_paths <- paste0("./", states, "/", states, csv_type)
  file_paths %>% 
    map_dfr(~read_csv(.x) %>%
...
districts <- st_read("./Shape_India/DISTRICT_BOUNDARY.shp")
```
