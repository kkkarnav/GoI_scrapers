...
def grab_month_data(village_row, finyear, month):
    voucherwise = "Y"
    schemewise = "P"
    schemecode = "-1"
...
def grab_data(village_row, finyear):
    ...
    for month in range(1, 13):
        ...
        try:
            target_table = grab_month_data(village_row, finyear, month)
        except:
            print(f"Error scraping {village_row['gp_name']}, in {village_row['district_name']} for {month} {finyear}")
            continue
        ...
        for row in target_table.find_all('tr')[3:-1]:
            ...
            if f"{row.find_all('td')[0].get_text(strip=True)}" != "":
                receipt = [row.find_all('td')[0].get_text(strip=True), "Receipt",
                           row.find_all('td')[2].get_text(strip=True),
                           row.find_all('td')[1].get_text(strip=True),
                           row.find_all('td')[1].find("a")["href"],
                           row.find_all('td')[3].get_text(strip=True)]
                entries.append(receipt)
            if f"{row.find_all('td')[4].get_text(strip=True)}" != "":
                payment = [row.find_all('td')[4].get_text(strip=True), "Payment",
                           row.find_all('td')[6].get_text(strip=True),
                           row.find_all('td')[5].get_text(strip=True),
                           row.find_all('td')[5].find("a")["href"],
                           row.find_all('td')[7].get_text(strip=True)]
                entries.append(payment)

    # print(opening_balances)
    village_df = pd.DataFrame(entries, columns=["date", "entry", "type", "code", "link", "amount"])
    ...
    village_df[["state_code", "state_name", "district_code", "district_name", "block_code", "block_name", "gp_code", "gp_name", "N_vil"]] = village_row.loc[["state_code", "state_name", "district_code", "district_name", "block_code", "block_name", "gp_code", "gp_name", "N_vil"]]
...
if __name__ == "__main__":
    ...
    for year in range(start_year, end_year):
        ...
        print(df)
        df.to_csv(f"./{state}_{year}_voucher_data.csv", index=False)
